...
### Обновление handoff (добавлено: 2026-02-22, вечер)

#### Что уже сделано в мастере создания (актуально)

- [x] Ранний выбор уровня перенесен в шаг `Origin` (влияет на спеллы/листы сразу).
- [x] Шаг `Spells` переведен на актуальные лимиты уровня:
  - [x] лимиты по количеству известных фокусов/заклинаний;
  - [x] ограничение по максимальному кругу от доступных ячеек;
  - [x] автоочистка невалидных выборов при смене уровня/класса.
- [x] Исправлен баг «можно выбрать бесконечно»:
  - [x] `limit=0` теперь трактуется как запрет выбора, не как безлимит.
- [x] Добавлены видимые счетчики лимитов на шаге заклинаний (`текущий/лимит`, остаток).
- [x] Заклинания в мастере отображаются табами по уровням:
  - [x] `Фокусы (заговоры)` отдельным табом;
  - [x] табы по кругам заклинаний.
- [x] Шаг `Abilities` улучшен:
  - [x] интерактивный стандартный массив без дублей;
  - [x] добавлена поддержка распределения бонусов предыстории `+2/+1` и `+1/+1/+1` (с пресетами и валидацией формы).
- [x] Финальный лист (`Review`) переработан:
  - [x] пояснения формул через tooltip (ХП/КД/золото);
  - [x] блок экономики старта (золото класса + предыстории).
- [x] Расчет стартового золота унифицирован:
  - [x] итоговый кошелек = `class gold (если выбран gold-alternative)` + `background startingGold`;
  - [x] убран риск двойного учета при переключениях в `EquipmentStep`.
- [x] В `Review` добавлены кликабельные ссылки на справочники:
  - [x] раса, класс, предыстория, черта предыстории, снаряжение, заклинания.
- [x] Добавлен контур возврата в мастер:
  - [x] `returnTo` на справочниках;
  - [x] deep-link в мастер по `?step=...`.

#### Что важно знать следующему агенту перед добавлением подклассов

1. Подклассы в БД **пока не подключены в creation flow** намеренно.
2. В сторе уже есть поле `subclassId`, но UI/валидация создания сейчас работают без обязательного выбора подкласса.
3. Уровень выбирается в `Origin`, поэтому все subclass-условия нужно проверять относительно `state.level` с раннего шага.
4. Ограничения спеллов уже завязаны на классовую progression-таблицу и уровень; при добавлении subclass spells нельзя ломать текущие лимиты base class.
5. `Review` и `CharacterSheetPreview` уже объясняют источники значений; subclass-бонусы нужно добавлять как отдельные источники, а не смешивать с базовыми.

#### Рекомендуемый порядок внедрения подклассов (для следующего агента)

1. Поднять данные:
   - заполнить `Subclass` и `SubclassFeature` по PHB2024;
   - проверить внешний ключ с `CharacterClass`.
2. Бэкенд-контракт:
   - убедиться, что `/classes/meta` и/или отдельный endpoint отдают минимум для выбора подкласса на нужном уровне;
   - не перегружать payload на ранних шагах (лучше lazy-load по выбранному классу).
3. Creation wizard:
   - показывать блок выбора подкласса только если `level >= subclassLevel`;
   - при смене класса/уровня очищать невалидный `subclassId`.
4. Валидация:
   - добавить проверку в `canContinue` для шага `class` (или отдельного шага), если подкласс обязателен на выбранном уровне.
5. Review/лист:
   - вывести выбранный подкласс;
   - показать subclass features текущего уровня(ей) с источником.
6. Edit flow:
   - разрешить менять `level` и `subclassId` согласованно;
   - при понижении уровня чистить невалидный подкласс/связанные выборы.

#### Риски при добавлении подклассов

- Не допустить регресса в текущем лимитировании спеллов.
- Не ломать возвраты `returnTo` и deep-link `?step=...`.
- Не переиспользовать старые «заглушки» subclass из legacy-таблиц без проверки PHB2024-актуальности.
- Любые сиды/апдейты по подклассам делать идемпотентно.
...
## PRIORITY: first tasks for next agent

Этот блок выполнить **в первую очередь**, до остальных пунктов плана.

### 1) Улучшить создание персонажа: черта от предыстории

- На шаге происхождения явно показывать, какую черту дает выбранная предыстория.
- Показывать краткое описание этой черты прямо в wizard (не только название).
- Название черты должно быть кликабельным и вести на `#/feats#<id>`.
- Если `originFeat` не резолвится в `Feat.id`, показывать понятный fallback и логировать проблему.

Критерий готовности:
- Для всех 16 PHB2024 background черта отображается корректно, на русском и со ссылкой.

### 2) Убрать английские типы/названия из UI, где возможно

- Провести ревизию экранов, которые уже менялись в рамках PHB2024:
  - создание персонажа (все шаги),
  - страницы классов/предысторий/снаряжения/черт.
- Русские подписи и значения брать из бэкенда (`nameRu`, русские поля/описания), а не из фронтовых мапперов-переводчиков.
- Оставлять английский только как fallback, если в БД реально нет русского значения.

Критерий готовности:
- В пользовательском флоу создания персонажа нет англоязычных “типов”/служебных названий при наличии русских данных в БД.

### 3) QA каждого шага мастера создания (bug sweep)

- Пройти вручную все шаги wizard:
  - Origin, Class, Abilities, Equipment, Spells, Details, Review.
- Проверить:
  - корректность ссылок (equipment/feats/spells),
  - корректность конфликтов навыков и компенсационных выборов,
  - соответствие логики PHB 2024 (выборы, ограничения, источники бонусов),
  - сохранение/редактирование персонажа для авторизованного пользователя.
- Зафиксировать найденные баги и исправить в этой же ветке.

Критерий готовности:
- Нет блокирующих багов по созданию персонажа;
- Приложен короткий чеклист “проверено/исправлено”.
